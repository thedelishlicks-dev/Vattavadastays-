...
import { format, addMonths, startOfMonth, endOfMonth, startOfWeek, endOfWeek, addDays, isAfter, isBefore, isSameDay, isSameMonth } from "date-fns";
      ...
      <div className="bg-card border border-border rounded-2xl w-full max-w-md shadow-xl">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <button onClick={() => setMonth(addMonths(month, -1))} className="h-8 w-8 flex items-center justify-center rounded-full hover:bg-muted">
                ...
                return (
                  <button
                    ...
                    className={[
                      ...
                      isSelected ? "bg-red-500 text-white" : (!isPast && !outOfMonth ? "hover:bg-muted" : ""),
                    ].join(" ")}
                  >
                    {format(d, "d")}
                  </button>
          ...
          {selected.size > 0 && (
            <p className="text-xs text-muted-foreground">
              {selected.size} date{selected.size > 1 ? "s" : ""} selected — will be marked unavailable
            </p>
          )}
        ...
        <div className="flex items-center justify-end gap-3 px-5 pb-5">
          ...
          >
            ...
            {done ? "Blocked ✓" : `Block ${selected.size > 0 ? selected.size : ""} date${selected.size !== 1 ? "s" : ""}`}
          </button>
        </div>
      </div>
    </div>
