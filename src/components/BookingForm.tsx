          ...
          <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-[var(--shadow-soft)]">
            <div className="rounded-xl bg-primary-light/40 p-4 mb-6 text-sm">
              ...
              <div className="mt-2 font-display text-xl font-semibold text-primary">
                Rs.{selection.total.toLocaleString("en-IN")}
              </div>
            </div>
              ...
              <div className="text-center py-6 space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-2 text-sm text-muted-foreground max-w-xs mx-auto">
                    {property?.owner_name ?? "The host"} will confirm your booking on WhatsApp shortly.
                  </p>
                </div>
                {whatsappBookingLink && (
                  <a href={whatsappBookingLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full bg-[#25D366] text-white px-6 py-3 text-sm font-medium hover:opacity-90 transition-opacity">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  ...
                  <Field label="Phone" required>
                    <input required type="tel" value={phone} onChange={(e) => setPhone(e.target.value)} className={inputCls} placeholder="+91 98765 43210" />
                  </Field>
                </div>
                ...
                <Field label="Special requests">
                  <textarea rows={3} value={requests} onChange={(e) => setRequests(e.target.value)} className={inputCls} placeholder="Arrival time, dietary needs, etc." />
                </Field>
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {(["UPI", "Bank Transfer", "Cash on Arrival"] as Payment[]).map((p) => (
                      <button type="button" key={p} onClick={() => setPayment(p)}
                        className={["rounded-lg border px-3 py-2 text-xs md:text-sm font-medium transition-colors",
                          payment === p ? "bg-primary text-primary-foreground border-primary" : "bg-background border-border hover:bg-accent"].join(" ")}>
                        {p}
                      </button>
                    ))}
                  </div>
                ...
                {whatsappBookingLink && (
                  <div className="rounded-xl border border-border bg-muted/30 p-4 text-center">
                    ...
                    <a href={whatsappBookingLink} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 rounded-full border border-[#25D366] text-[#128C7E] px-4 py-2 text-xs font-medium hover:bg-[#25D366]/10 transition-colors">
                ...
                <button type="submit" disabled={isPending} className="w-full rounded-full bg-primary py-4 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">
