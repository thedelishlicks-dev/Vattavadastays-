      ...
      <div className="flex items-center justify-between pt-3 border-t border-border">
        <div className="text-xs text-muted-foreground">
          ...
          <span className="font-semibold text-foreground">
            ₹{(Number(booking.total_amount) - discount).toLocaleString("en-IN")}
          </span>
          {discount > 0 && (
            <span className="ml-1.5 text-xs text-green-600 font-medium">-₹{discount.toLocaleString("en-IN")} disc</span>
          )}
        </div>
        {isActive && (
          advance > 0 ? (
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center">
      ...
      <div className="relative w-full md:max-w-lg bg-card rounded-t-3xl md:rounded-2xl shadow-2xl max-h-[92vh] flex flex-col">
        ...
        <div className="px-5 pt-3 pb-4 border-b border-border">
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            ...
            {booking.status === "confirmed" && (
              <ActionChip onClick={() => handleStatus("completed")} loading={updating} icon={<LogOut className="h-3.5 w-3.5" />} label="Complete" color="amber" />
            )}
        ...
        <div className="flex-1 overflow-y-auto">
          {tab === "overview" && (
            <OverviewTab booking={booking} roomName={roomName} property={property} advance={advance} discount={discount} balance={balance} chargesTotal={chargesTotal} onPaymentSaved={onPaymentSaved} ownerPhone={ownerPhone} upiId={upiId} />
          )}
          {tab === "charges" && (
            <ChargesTab booking={booking} charges={charges} chargesTotal={chargesTotal} discount={discount} advance={advance} balance={balance} />
          )}
          {tab === "invoice" && (
            <div className="p-5">
              <BookingInvoice booking={booking} roomName={roomName} property={property ?? null} charges={charges} chargesTotal={chargesTotal} advance={advance} balance={balance} guestView={false} />
            </div>
          )}
        </div>
      </div>
...
function DiscountForm({ booking, discount, onSaved, onCancel }: { booking: Booking; discount: number; onSaved: () => void; onCancel: () => void }) {
  ...
  const handleSave = async () => {
    ...
    if (amt < 0) { setError("Discount cannot be negative"); return; }
    if (amt > Number(booking.total_amount)) { setError("Discount cannot exceed total amount"); return; }
    setSaving(true);
    setError("");
    try {
      const { error: err } = await supabase
        .from("bookings")
        .update({ discount_amount: amt, discount_reason: reason.trim() || null })
        .eq("id", booking.id);
      if (err) throw err;
  ...
  const handleRemove = async () => {
    setSaving(true);
    await supabase.from("bookings").update({ discount_amount: 0, discount_reason: null }).eq("id", booking.id);
    queryClient.invalidateQueries({ queryKey: ["bookings"], exact: false });
    onSaved();
  };
  ...
  return (
    <div className="rounded-xl border border-green-200 bg-green-50/50 p-4 space-y-3">
      ...
      <div>
        ...
        <input type="number" min={0} max={Number(booking.total_amount)} value={amount} onChange={(e) => setAmount(e.target.value)} className={inputCls} placeholder="e.g. 500" autoFocus />
      </div>
      <div>
        <label className={labelCls}>Reason (optional)</label>
        <input value={reason} onChange={(e) => setReason(e.target.value)} className={inputCls} placeholder="e.g. Regular guest, Long stay" />
      </div>
      {error && <p className="text-xs text-destructive">{error}</p>}
      <div className="flex gap-2">
        {discount > 0 && (
          <button onClick={handleRemove} disabled={saving} className="px-3 rounded-full border border-destructive/30 text-destructive text-xs py-2 hover:bg-destructive/5 disabled:opacity-50">Remove</button>
        )}
        <button onClick={onCancel} className="flex-1 rounded-full border border-border py-2 text-sm hover:bg-muted">Cancel</button>
        <button onClick={handleSave} disabled={saving} className="flex-1 rounded-full bg-green-600 text-white py-2 text-sm font-medium hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-1">
...
function OverviewTab({
  booking, roomName, property, advance, discount, balance, chargesTotal, onPaymentSaved, ownerPhone, upiId,
}: {
  booking: Booking;
  roomName: string;
  property: ReturnType<typeof useOwnerProperty>["data"];
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      <Section title="Payment summary">
        ...
        {discount > 0 && (
          <Row label={`Discount${booking.discount_reason ? ` (${booking.discount_reason})` : ""}`} value={`-₹${discount.toLocaleString("en-IN")}`} highlight="green" />
        )}
      ...
      {showDiscountForm && (
        <DiscountForm booking={booking} discount={discount} onSaved={() => { setShowDiscountForm(false); onPaymentSaved(); }} onCancel={() => setShowDiscountForm(false)} />
      )}
      ...
      {showPaymentForm && (
        <RecordPaymentForm booking={booking} advance={advance} discount={discount} onSaved={() => { setShowPaymentForm(false); onPaymentSaved(); }} onCancel={() => setShowPaymentForm(false)} />
      )}
...
function RecordPaymentForm({ booking, advance, discount, onSaved, onCancel }: { booking: Booking; advance: number; discount: number; onSaved: () => void; onCancel: () => void }) {
  ...
  const handleSave = async () => {
    ...
    if (newPayment > maxAllowed) {
      setError(maxAllowed <= 0 ? "This booking is already fully paid" : `Maximum you can record is ₹${maxAllowed.toLocaleString("en-IN")} (remaining balance)`);
      return;
    }
    setSaving(true);
    setError("");
    try {
      const { error: err } = await supabase.from("bookings").update({
        ...
        is_paid: newAdvanceTotal >= Number(booking.total_amount) - discount,
  ...
  return (
    <div className="rounded-xl border border-primary/20 bg-primary-light/20 p-4 space-y-3">
      ...
      <div>
        <label className={labelCls}>{advance > 0 ? "New payment received (₹) *" : "Amount received (₹) *"}</label>
        <input type="number" min={0} max={maxAllowed} value={amount} onChange={(e) => setAmount(e.target.value)} className={inputCls} placeholder={advance > 0 ? "This instalment only" : "Actual amount received"} autoFocus />
      </div>
      ...
      {newPayment > 0 && (
        <div className="rounded-lg bg-background px-3 py-2 space-y-1">
          ...
          <div className="flex justify-between text-sm">
            ...
            <span className={`font-semibold ${bal === 0 ? "text-primary" : "text-amber-700"}`}>{bal === 0 ? "Fully paid ✓" : `₹${bal.toLocaleString("en-IN")}`}</span>
          </div>
        </div>
      )}
      {error && <p className="text-xs text-destructive">{error}</p>}
...
function ChargesTab({ booking, charges, chargesTotal, discount, advance, balance }: { booking: Booking; charges: BookingCharge[]; chargesTotal: number; discount: number; advance: number; balance: number }) {
  ...
  return (
    <div className="p-5 space-y-4">
      ...
      {discount > 0 && (
        <div className="rounded-xl bg-green-50 border border-green-200 px-4 py-2 flex justify-between text-sm">
...
function AddBookingModal({ propertyId, rooms, onClose, onSaved }: { propertyId: string; rooms: { id: string; name: string; base_price: number; extra_guest_price: number }[]; onClose: () => void; onSaved: () => void }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const { error: err } = await supabase.from("bookings").insert({
        ...
        guest_phone: form.guest_phone, guest_count: form.guest_count, check_in: form.check_in,
        check_out: form.check_out, room_price: (selectedRoom?.base_price ?? 0) * nights,
        extra_guest_charge: Math.max(0, form.guest_count - 2) * (selectedRoom?.extra_guest_price ?? 0) * nights,
        total_amount: total, advance_amount: 0, discount_amount: 0, status: form.status, is_paid: false,
      });
      if (err) throw err;
      onSaved();
    } catch (e: unknown) {
...
function BookingsAdmin() {
  ...
  const stats = useMemo(() => {
    ...
    const outstanding = active.reduce((s, b) => s + Math.max(0, Number(b.total_amount) - Number(b.discount_amount ?? 0) - Number(b.advance_amount ?? 0)), 0);
  ...
  const updateStatus = async (id: string, newStatus: string) => {
    ...
    if (newStatus === "confirmed" && property) {
      ...
      if (booking) {
        ...
        a.href = waUrl; a.target = "_blank"; a.rel = "noreferrer";
  ...
  return (
    <div className="space-y-5">
      ...
      {showAdd && property && (
        <AddBookingModal propertyId={property.id} rooms={rooms.filter((r) => r.is_active)} onClose={() => setShowAdd(false)} onSaved={() => { queryClient.invalidateQueries({ queryKey: ["bookings", property?.id], exact: false }); setShowAdd(false); }} />
      )}
