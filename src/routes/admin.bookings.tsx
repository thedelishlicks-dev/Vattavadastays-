    ...
    >
      ...
      <div className="grid grid-cols-2 gap-2 mb-3">
        ...
        <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
          ...
          <span>
            {booking.check_out} · {booking.nights}N
          </span>
        </div>
      </div>
      ...
      <div className="relative w-full md:max-w-lg bg-card rounded-t-3xl md:rounded-2xl shadow-2xl max-h-[92vh] flex flex-col">
        ...
        <div className="px-5 pt-3 pb-4 border-b border-border">
          <div className="flex items-start justify-between">
            ...
            <button
              onClick={onClose}
              className="h-8 w-8 rounded-full hover:bg-muted flex items-center justify-center -mt-1"
            >
              <X className="h-4 w-4" />
            </button>
          </div>
          ...
          <div className="flex gap-2 mt-3 flex-wrap">
            ...
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="inline-flex items-center gap-1.5 rounded-full border border-[#25D366]/40 bg-[#25D366]/5 text-[#128C7E] px-3 py-1.5 text-xs font-medium hover:bg-[#25D366]/10 transition-colors"
            >
...
function ActionChip({
  ...
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className={`inline-flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium transition-colors disabled:opacity-60 ${colors[color]}`}
    >
    ...
    <div className="p-5 space-y-4">
      <Section title="Stay details">
        ...
        {booking.checked_in_at && (
          <Row
            label="Checked in at"
            value={new Date(booking.checked_in_at).toLocaleString("en-IN", {
              dateStyle: "medium",
              timeStyle: "short",
            })}
          />
        ...
        {booking.checked_out_at && (
          <Row
            ...
            value={new Date(booking.checked_out_at).toLocaleString("en-IN", {
              dateStyle: "medium",
              timeStyle: "short",
            })}
          />
      ...
      <Section title="Payment summary">
        ...
        />
        {chargesTotal > 0 && (
          <Row label="Extra charges" value={`₹${chargesTotal.toLocaleString("en-IN")}`} />
        )}
        {advance > 0 && (
          <Row
            label="Advance paid"
        ...
        {booking.payment_reference && (
          <Row label="Reference" value={booking.payment_reference} small />
        )}
        <div className="border-t border-border pt-2 mt-1">
          <Row
            label="Balance due"
            value={balance === 0 ? "Fully paid ✓" : `₹${balance.toLocaleString("en-IN")}`}
            highlight={balance === 0 ? "green" : "amber"}
            bold
          />
        </div>
      </Section>
  ...
  if (!confirming)
    return (
      <button
        ...
        className="w-full rounded-xl border border-destructive/20 py-2.5 text-sm text-destructive hover:bg-destructive/5 transition-colors"
      >
        Cancel booking
      </button>
    );
    ...
    <div className="rounded-xl border border-destructive/30 bg-destructive/5 p-3 space-y-2">
      ...
      <div className="flex gap-2">
        ...
        <button
          ...
          disabled={loading}
          className="flex-1 rounded-lg bg-destructive text-white py-2 text-sm hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-1"
        >
...
function ChargesTab({
  ...
  const handleAdd = async () => {
    ...
    if (!q || q <= 0 || !p || p < 0) {
      setError("Valid qty and price required");
      return;
    }
    setError("");
    try {
    ...
    <div className="p-5 space-y-4">
      <div className="grid grid-cols-3 gap-2">
        ...
        <div className="rounded-xl bg-amber-50 border border-amber-100 p-3 text-center">
          ...
          <div className="font-semibold text-sm mt-0.5 text-amber-700">
        ...
        <div className="rounded-xl bg-primary-light/40 border border-border p-3 text-center">
          ...
          <div
            className={`font-semibold text-sm mt-0.5 ${balance === 0 ? "text-primary" : "text-amber-700"}`}
          >
      ...
      {charges.length === 0 ? (
        ...
        <div className="space-y-2">
          {charges.map((c) => (
            <div
              key={c.id}
              className="flex items-center gap-3 rounded-xl bg-muted/30 border border-border px-4 py-3"
            >
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{c.description}</div>
                <div className="text-xs text-muted-foreground mt-0.5">
                  {c.qty > 1
                    ? `${c.qty} × ₹${c.unit_price.toLocaleString("en-IN")}`
  ...
  const handleSave = async () => {
    ...
    try {
      const { error: err } = await supabase.from("bookings").insert({
        ...
        extra_guest_charge:
          Math.max(0, form.guest_count - 2) * (selectedRoom?.extra_guest_price ?? 0) * nights,
        total_amount: total,
        advance_amount: 0,
        status: form.status,
        is_paid: false,
          ...
          {nights > 0 && selectedRoom && (
            <div className="rounded-xl bg-primary-light/40 border border-border p-3 flex justify-between text-sm">
              <span className="text-muted-foreground">
                {nights}N · {form.guest_count} guests
              </span>
  ...
  const filtered = useMemo(
    () =>
      bookings.filter((b) => {
        ...
        if (q && !`${b.guest_name} ${b.guest_phone}`.toLowerCase().includes(q.toLowerCase()))
          return false;
        return true;
      }),
    ...
    <div className="space-y-5">
        ...
        <div className="space-y-3">
          {filtered.map((b) => (
            <BookingCard
              ...
              roomName={roomNameMap[b.room_id] ?? "Unknown room"}
              onClick={() => setActiveBooking(b as Booking)}
            />
          ))}
        </div>
      )}
      ...
      {activeBooking && (
        <BookingDetailModal
          ...
          roomName={roomNameMap[activeBooking.room_id] ?? "Unknown room"}
          property={property}
          onClose={() => setActiveBooking(null)}
          onStatusChange={updateStatus}
          onPaymentSaved={handleRefresh}
        />
      )}
    </div>
  );
